def echelon_form(matrix):
    """
    Converts an mxn matrix to its echelon form using Gaussian elimination.
    """
    rows = len(matrix)
    cols = len(matrix[0])
    pivot_row = 0
    ...
    for j in range(cols):
        ...
        if pivot_found:
            ...
            # Use the pivot row to eliminate the elements below it in the column
...
def display(matrix):
    echelonMatrix=echelon_form(matrix)
    print(echelonMatrix)
